...
$Icon                                = New-Object system.drawing.icon ("https://cdn.icon-icons.com/icons2/48/PNG/512/printer_folder_file_10495.png")
...
$Description.text                = "To add a printer, fill-in the boxes with the needed information.."
...
@('Xerox','HP', 'Zebra') | ForEach-Object {[void] $PrinterType.Items.Add($_)}
...
function AddPrinter { 
  ...
  if ($PrinterType.SelectedItem -eq 'Xerox') {
    $printerDriverName = "Xerox Global Print Driver PCL6"
  }elseif ($PrinterType.SelectedItem -eq 'Zebra') {
    $printerDriverName = "ZDesigner GK420d"
  }elseif ($PrinterType.SelectedItem -eq 'HP') {
    $printerDriverName = "HP Universal Printing PS (v7.0.1)"
  }
  else{
    $printerDriverName = "HP Universal Printing PS (v7.0.1)"
  }
