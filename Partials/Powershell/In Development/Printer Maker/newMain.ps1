...
function PrintServerInfo {
    ...
    process {
        foreach ($i in $PrintServers) {
                try {
                    $result = (get-printer -ComputerName $i |  Where-Object {($_.Name -like '*'+$userinputPrinterName+'*')})
                ...
                if ($result.Status -eq 'Success') {
                    ...
                    $PrinterName = $result.Name}else {
                        Write-Warning "Machine '$printer' may not be on Printe Sever." 
                }
        }
    }
}
