...
jsonfile = tempdir+"variables.json"#'Partials/Python/PowerShellScripts.json'
jsonfile_pass = "$jsonfile = \""+jsonfile+"\""

vars_to_PS = jsonfile_pass
...
power_shell_resolve_EE = {"ResolveHostname" :vars_to_PS+"""
    ...
    $EE = "*"+$EE # This is to add the wild card to the front and not have to quote in the search
    ...
    if (Get-Module -ListAvailable -Name activedirectory) {
        ...
        Else{
            ...
            $HostnameSearch_lvl1 = Get-ADComputer -Filter * -SearchBase $OUSearchScope | Where-Object {$_.Name -like $EE} | Select-Object -ExpandProperty Name #Searches : OU=Laptops,OU=Lexington (LEX),OU=VISN09,DC=v09,DC=med,DC=va,DC=gov
            if (!$HostnameSearch_lvl1) {
                Write-Host "The EE $EE could be found in OU: $OUSearchScope" -foreground Red
                $HostnameSearch_lvl2 = Get-ADComputer -Filter * -SearchBase $OUSearchScope.Split(",", 2)[1] | Where-Object {$_.Name -like $EE} | Select-Object -ExpandProperty Name #Searches : OU=Lexington (LEX),OU=VISN09,DC=v09,DC=med,DC=va,DC=gov
                if (!$HostnameSearch_lvl2) {
                    ...
                    $HostnameSearch_lvl3 = Get-ADComputer -Filter * -SearchBase $OUSearchScope.Split(",", 3)[2] | Where-Object {$_.Name -like $EE} | Select-Object -ExpandProperty Name #Searches : OU=VISN09,DC=v09,DC=med,DC=va,DC=gov
                                
                    if (!$HostnameSearch_lvl3) {
                        $Hostname = $EE
                        #$HostnameArray.Add($Hostname) > $null
                        #$ADLocationArray.Add("Not Found") > $null
                        Write-Host "Extended search found $EE could not be located in OU: " $OUSearchScope.Split(",", 3)[2] -foreground Red #NEVER Found
                    }
                    else {
                        $Hostname = $HostnameSearch_lvl3
                        #$HostnameArray.Add($Hostname) > $null
                        #$ADLocationArray.Add($OUSearchScope.Split(",", 3)[2]) > $null
                        Write-Host "Extended search found $EE resolves to $Hostname It was in OU: " $OUSearchScope.Split(",", 3)[2] -foreground Green #Found in OU: OU=VISN09,DC=v09,DC=med,DC=va,DC=gov
                    }
                }
                else {
                    ...
                    Write-Host "Extended search found $EE resolves to $Hostname It was in OU: " $OUSearchScope.Split(",", 2)[1] -foreground Green #Found in OU: OU=Lexington (LEX),OU=VISN09,DC=v09,DC=med,DC=va,DC=gov
                }
            }       
            else {
                ...
                Write-Host "Extended search found $EE resolves to $Hostname" -foreground Green #Found in : OU=Laptops,OU=Lexington (LEX),OU=VISN09,DC=v09,DC=med,DC=va,DC=gov
            }
            
            #Sources:
            #https://adamtheautomator.com/powershell-parameter/
        }
        $Hostname = Get-ADComputer -Filter * -SearchBase $OUSearchScope | Where-Object {$_.Name -like $EE} | Select-Object -ExpandProperty Name
    } 
    else{
