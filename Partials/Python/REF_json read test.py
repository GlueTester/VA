...
power_shell_resolve_EE = {
  "ResolveHostname" :"""
    $envfile = "C:\\temp\.env"
    $hostnamefile = "C:\\temp\hsotname.txt"
    $SearchType = $env:SearchType
    #[Environment]::SetEnvironmentVariable("EE",$env:EE)
    $ImportedEE = $env:EE
    echo $env:EE
    if ( $ImportedEE -eq $null){
        write-host "EE is blank, ENV EE is $env:EE"
        exit
    }
    ...
    ls Env:
    [Environment]::SetEnvironmentVariable("EE",$null)
    ls Env:
    echo "Hostname = \'$Hostname\'" | out-file -filepath $envfile -append -Encoding ASCII #Thsi writes it to the .env file
    # echo "Hostname = \'$Hostname\'" | out-file -filepath $hostnamefile -append -Encoding ASCII
    echo "EE is $EE"| out-file -filepath $hostnamefile -append -Encoding ASCII
    """,
"name":"new guy"
}


#with open('Partials/Python/PowerShellScripts.json', 'w') as f:
#  json.dump(power_shell_resolve_EE , f)
  

with open('Partials/Python/PowerShellScripts.json', 'r') as f:
  data = json.load(f)
#print(data)
